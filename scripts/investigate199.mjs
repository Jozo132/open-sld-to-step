#!/usr/bin/env bun
/**
 * investigate199.mjs — Search every planar face loop for a match to the FTC_07 broad-133 sequence.
 *
 * Goal:
 * 1. Decode the ordered broad-133 segment sequence from the FTC_07 slot-3 branch.
 * 2. Scan both outer and inner bounds of every planar ADVANCED_FACE in the
 *    reference STEP file.
...
planarFaceLoops.sort((left, right) => left.score - right.score);

console.log('investigate199 — FTC_07 broad-133 loop vs all planar face bounds');
console.log('Clean-room basis: search both outer and inner planar face loops for the closest length signature match to broad-133.');
...
console.log('\nTop planar loop matches:');
for (const face of planarFaceLoops.slice(0, 20)) {
    ...
    console.log(`  face=#${face.faceId} plane=#${face.planeId} ${face.boundType}Bound=#${face.boundId} loop=#${face.loopId} edges=${face.faceEdges.length} score=${formatNumber(face.score)} origin=${origin} axis=${axis}`);
